...
export async function applyAuthChoiceApiProviders(
  ...
  if (
    ...
    } else if (params.opts.tokenProvider === "opengateway") {
      authChoice = "opengateway-api-key";
  ...
  if (authChoice === "opengateway-api-key") {
    ...
    const existingProfileId = profileOrder.find((profileId) => Boolean(store.profiles[profileId]));
    const existingCred = existingProfileId ? store.profiles[existingProfileId] : undefined;
    let profileId = "opengateway:default";
    let mode: "api_key" | "oauth" | "token" = "api_key";
    let hasCredential = false;
    ...
    if (!hasCredential && params.opts?.token && params.opts?.tokenProvider === "opengateway") {
      await setOpengatewayApiKey(normalizeApiKeyInput(params.opts.token), params.agentDir);
      hasCredential = true;
    }

    if (!hasCredential) {
      const envKey = resolveEnvApiKey("opengateway");
      if (envKey) {
        const useExisting = await params.prompter.confirm({
          message: `Use existing OPENGATEWAY_API_KEY (${envKey.source}, ${formatApiKeyPreview(envKey.apiKey)})?`,
          initialValue: true,
        });
        if (useExisting) {
          await setOpengatewayApiKey(envKey.apiKey, params.agentDir);
          hasCredential = true;
        }
      }
    }
    ...
    if (!hasCredential) {
      const key = await params.prompter.text({
        message: "Enter OpenGateway API key",
        validate: validateApiKeyInput,
      });
      await setOpengatewayApiKey(normalizeApiKeyInput(String(key)), params.agentDir);
      hasCredential = true;
    }
    ...
    }
    {
      const applied = await applyDefaultModelChoice({
        config: nextConfig,
        setDefaultModel: params.setDefaultModel,
        defaultModel: OPENGATEWAY_DEFAULT_MODEL_REF,
        applyDefaultConfig: applyOpengatewayConfig,
        applyProviderConfig: applyOpengatewayProviderConfig,
        noteDefault: OPENGATEWAY_DEFAULT_MODEL_REF,
        noteAgentModel,
        prompter: params.prompter,
      });
      nextConfig = applied.config;
