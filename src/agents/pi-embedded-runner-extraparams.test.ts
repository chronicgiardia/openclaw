...
describe("applyExtraParamsToAgent", () => {
  ...
  it("normalizes official Anthropic custom tools to explicit custom format", () => {
    const payloads: Record<string, unknown>[] = [];
    const baseStreamFn: StreamFn = (_model, _context, options) => {
      const payload: Record<string, unknown> = {
        tools: [
  ...
  it("does not rewrite anthropic tool schema for custom anthropic-compatible endpoints", () => {
    const payloads: Record<string, unknown>[] = [];
    const baseStreamFn: StreamFn = (_model, _context, options) => {
      const payload: Record<string, unknown> = {
        tools: [
  ...
  it("supports config override to disable Anthropic tool signature rewrites", () => {
    const payloads: Record<string, unknown>[] = [];
    const baseStreamFn: StreamFn = (_model, _context, options) => {
      const payload: Record<string, unknown> = {
        tools: [
    ...
    applyExtraParamsToAgent(
      agent,
      buildAnthropicModelConfig("anthropic/claude-sonnet-4-6", {
        anthropicToolSignatureExperiment: "off",
      }),
      "anthropic",
      "claude-sonnet-4-6",
      undefined,
  ...
  it("supports config override to force a specific Anthropic tool signature experiment", () => {
    const payloads: Record<string, unknown>[] = [];
    const baseStreamFn: StreamFn = (_model, _context, options) => {
      const payload: Record<string, unknown> = {
        tools: [
    ...
    applyExtraParamsToAgent(
      agent,
      buildAnthropicModelConfig("anthropic/claude-sonnet-4-6", {
        anthropicToolSignatureExperiment: "openai-functions-compact",
      }),
      "anthropic",
      "claude-sonnet-4-6",
      undefined,
