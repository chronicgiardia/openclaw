...
function isOfficialAnthropicMessagesEndpoint(model: {
  api?: unknown;
  provider?: unknown;
  baseUrl?: unknown;
}): boolean {
...
function resolveAnthropicToolSignatureExperimentId(params: {
  api?: unknown;
  provider?: unknown;
  baseUrl?: unknown;
  configuredExperiment?: unknown;
}): AnthropicToolSignatureExperiment["id"] | undefined {
  const configuredExperiment = resolveConfiguredAnthropicToolSignatureExperiment(
    params.configuredExperiment,
  );
  if (configuredExperiment !== undefined) {
    return configuredExperiment ?? undefined;
  }
...
function resolveConfiguredAnthropicToolSignatureExperiment(
  value: unknown,
  ...
  if (typeof value !== "string") {
    log.warn("ignoring non-string anthropicToolSignatureExperiment override");
    return undefined;
  }
  ...
  log.warn(`ignoring unknown anthropicToolSignatureExperiment override: ${trimmed}`);
  return undefined;
}

function replacePayloadContents(
...
/**
 * Some Anthropic-compatible endpoints expect non-default tool signatures:
 * - Kimi Coding expects OpenAI-style `type:function` tools.
 * - Anthropic's official Messages API accepts custom tools more reliably when
 *   they are emitted with an explicit `type:"custom"`.
 */
function createAnthropicToolSignatureWrapper(
  baseStreamFn: StreamFn | undefined,
  configuredExperiment?: unknown,
): StreamFn {
  const underlying = baseStreamFn ?? streamSimple;
    ...
    return underlying(model, context, {
      ...
      onPayload: (payload) => {
        if (payload && typeof payload === "object" && !Array.isArray(payload)) {
          const payloadObj = payload as Record<string, unknown>;
          const runtimeExperimentId = resolveAnthropicToolSignatureExperimentId({
            ...model,
            configuredExperiment,
          });
          if (runtimeExperimentId) {
            replacePayloadContents(
              payloadObj,
              applyAnthropicToolSignatureExperiment(payloadObj, runtimeExperimentId),
            );
          }
        }
        originalOnPayload?.(payload);
...
export function applyExtraParamsToAgent(
  ...
  agent.streamFn = createAnthropicToolSignatureWrapper(
    agent.streamFn,
    merged?.anthropicToolSignatureExperiment,
  );
