...
describe("buildAgentSystemPrompt", () => {
  ...
  it("omits extended sections in minimal prompt mode", () => {
    ...
    expect(prompt).toContain(
      "For long waits, avoid rapid poll loops: use exec with enough yieldMs/background and continue once the command is ready.",
    );
  ...
  it("redacts OpenClaw branding in Anthropic OAuth strict-compat mode", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/openclaw",
    ...
    expect(prompt).not.toContain("OpenClaw docs:");
